...
const ACCENT = "184, 255, 102"; // --accent as an rgb triplet, for alpha compositing
...
export function TourGlobe({
  ...
  useEffect(() => {
    ...
    for (let lat = -78; lat <= 84; lat += 2.6) {
      const spacing = 2.6 / Math.max(Math.cos(lat * DEG), 0.18);
    ...
    const drawSphere = () => {
      ...
      halo.addColorStop(0.55, "rgba(184, 255, 102, 0.035)");
    ...
    const drawLand = () => {
      ...
      for (let b = 0; b < BUCKETS; b += 1) {
        ...
        ctx.fillStyle = `rgba(255, 255, 255, ${(0.1 + depth * 0.3).toFixed(3)})`;
    ...
    const drawMarker = (
      ...
      const facing = Math.min(1, p.z * 2.6);
    ...
    const frame = (ts: number) => {
      ...
      legs.forEach((leg, index) => {
        ...
        strokeLeg(leg, 1, touchesActive ? 0.5 : 0.14, touchesActive ? 1.4 : 1);
      });

      if (!reduceMotion && legs.length) {
      ...
      tourCities.forEach((city) => {
        drawMarker(city, time, active?.name === city.name, tourRoute.includes(city.name));
      });
